...
where
    Stats: IntoIterator<Item = &'a Stmt>,
{
    ctx.go_into_block(|ctx| {
        ...
        for fn_decl in fn_decls.iter() {
            ...
            /* TODO in strict mode, functions are block-scoped, otherwise it's a nightmare */
            let flow = ctx.declare_name(&fn_decl.ident.sym.to_string(), varname, true);
